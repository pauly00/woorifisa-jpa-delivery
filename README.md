...
erDiagram
    ...
    Menu {
        ...
        String name
        int price
        Long store_id FK "연관관계의 주인"
    }
    
    Orders {
        Long id PK
        LocalDateTime orderDate
        OrderStatus status "ORDER, CANCEL"
        Long member_id FK "연관관계의 주인"
        Long menu_id FK "연관관계의 주인"
    }

    %% 연관관계 매핑 (1:N)
    Member ||--o{ Orders : "주문 (1:N)"
    Store ||--o{ Menu : "가게 메뉴 (1:N)"
    Menu ||--o{ Orders : "주문된 메뉴 (1:N)"
```
...
* **연관관계 편의 메서드:** 객체 양방향 관계에서 어느 한쪽만 데이터를 세팅하는 실수를 방지하기 위해 `Member.addOrder()`, `Orders.setMember()`, `Menu.confirmStore()` 등의 편의 메서드를 엔티티 내부에 구현하여 객체 지향적인 설계를 유지했습니다.
